...
public interface RatingRepository extends JpaRepository<RatingTable, Long> {
    ...
    @Query("SELECT new com.example.ratingsservice.models.AVGRating(r.movieId, AVG(r.rating)) FROM RatingTable r GROUP BY r.movieId ORDER BY AVG(r.rating) DESC")
    List<AVGRating> findTopRatings();
}
